...
MONGO_URI = os.environ.get(
    "MONGO_URI",
    "mongodb+srv://deepanahushekhar2007_db_user:<FRVH0jxkFO5A0OTQM>@spidy.kz0u8kf.mongodb.net/?appName=Spidy"
)
...
def is_subscribed(uid):
    ...
    data = subs_col.find_one({"uid": uid})
    if not data:
        return False

    if datetime.now() > data["expiry"]:
        subs_col.delete_one({"uid": uid})
        return False
