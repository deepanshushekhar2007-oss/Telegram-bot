...
bot_token = "8302113399:AAGSyB0q7u1ObWL2UtUszhbONcGODef81kI"       # PUT BOT TOKEN
ADMIN_ID = 6860983540
...
MONGO_URI = "mongodb+srv://deepanahushekhar2007_db_user:<FRVH0jxkFO5A0OTQM>@spidy.kz0u8kf.mongodb.net/?appName=Spidy"  # Replace with your MongoDB URI
...
free_mode_col = db["free_mode"]
group_access_col = db["group_access"]
users_col = db["users"]
user_join_col = db["user_join"]
...
subscriptions = {}   # user_id -> expiry datetime (loaded from MongoDB)
user_state = {}
user_files = {}
user_data = {}
...
def load_subscriptions():
    ...
    subscriptions = {}
    for doc in subscriptions_col.find():
        subscriptions[int(doc['user_id'])] = datetime.fromisoformat(doc['expiry'])

def save_subscriptions():
    subscriptions_col.drop()  # Clear collection
    for uid, exp in subscriptions.items():
        subscriptions_col.insert_one({'user_id': str(uid), 'expiry': exp.isoformat()})

def load_free_mode():
    global free_mode
    doc = free_mode_col.find_one()
    free_mode = doc['free_mode'] if doc else False

def save_free_mode():
    free_mode_col.drop()
    free_mode_col.insert_one({'free_mode': free_mode})

def load_group_access():
    global group_access
    group_access = set()
    for doc in group_access_col.find():
        group_access.add(int(doc['chat_id']))

def save_group_access():
    group_access_col.drop()
    for chat_id in group_access:
        group_access_col.insert_one({'chat_id': str(chat_id)})

def load_users():
    global all_users
    all_users = set()
    for doc in users_col.find():
        all_users.add(int(doc['user_id']))

def save_users():
    users_col.drop()
    for uid in all_users:
        users_col.insert_one({'user_id': str(uid)})

def load_user_join():
    global user_join_date
    user_join_date = {}
    for doc in user_join_col.find():
        user_join_date[int(doc['user_id'])] = datetime.fromisoformat(doc['join_date'])

def save_user_join():
    user_join_col.drop()
    for uid, dt in user_join_date.items():
        user_join_col.insert_one({'user_id': str(uid), 'join_date': dt.isoformat()})

def load_force_sub():
    global force_sub_enabled
    doc = force_sub_col.find_one()
    force_sub_enabled = doc['enabled'] if doc else False

def save_force_sub():
    force_sub_col.drop()
    force_sub_col.insert_one({'enabled': force_sub_enabled})
...
async def start(event):
    ...
    # ❌ Block if not subscribed & free mode OFF
    ...
    # ✅ SINGLE & FINAL ADMIN LOG (NO DUPLICATE)
    await notify_admin(
        "✨ **Bot Started** ✨\n\n"
