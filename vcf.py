...
# ===================== MONGODB PATCH (ADDED) =====================
# ONLY DATABASE LAYER REPLACED – LOGIC UNCHANGED
...
MONGO_URI = os.environ.get("mongodb+srv://deepanahushekhar2007_db_user:<FRVH0jxkFO5A0OTQM>@spidy.kz0u8kf.mongodb.net/?appName=Spidy")
mongo = MongoClient(MONGO_URI)
mdb = mongo["spidy_vcf_bot"]

col_users = mdb.users
col_subs = mdb.subscriptions
col_settings = mdb.settings
col_group_access = mdb.group_access
col_user_join = mdb.user_join
col_usage_total = mdb.usage_total
col_usage_daily = mdb.usage_daily
col_errors = mdb.errors


def load_users():
    global all_users
    all_users = set(u["_id"] for u in col_users.find())


def save_users():
    col_users.delete_many({})
    if all_users:
        col_users.insert_many([{"_id": u} for u in all_users])


def load_subscriptions():
    global subscriptions
    subscriptions = {}
    for s in col_subs.find():
        subscriptions[s["_id"]] = s["expiry"]


def save_subscriptions():
    col_subs.delete_many({})
    for uid, expiry in subscriptions.items():
        col_subs.insert_one({"_id": uid, "expiry": expiry})


def load_free_mode():
    global free_mode
    doc = col_settings.find_one({"_id": "free_mode"})
    free_mode = doc["value"] if doc else False
...
def load_force_sub():
    ...
    doc = col_settings.find_one({"_id": "force_sub"})
    force_sub_enabled = doc["value"] if doc else False


def save_force_sub():
    col_settings.update_one(
        {"_id": "force_sub"},
        {"$set": {"value": force_sub_enabled}},
        upsert=True
    )
...
def load_group_access():
    global group_access
    group_access = {}
    for g in col_group_access.find():
        group_access[g["_id"]] = g["value"]


def save_group_access():
    col_group_access.delete_many({})
    for k, v in group_access.items():
        col_group_access.insert_one({"_id": k, "value": v})


def load_user_join():
    global user_join
    user_join = set(u["_id"] for u in col_user_join.find())


def save_user_join():
    col_user_join.delete_many({})
    if user_join:
        col_user_join.insert_many([{"_id": u} for u in user_join])
...
➤ /health  
   ┗ 🩺 View bot health, uptime & usage report
...
✨ Premium & VIP Options  
✨ Live Bot Health & Usage Tracking  
...
def inc_total(feature=None, files=0):
    col_usage_total.update_one(
        {"_id": "total"},
        {
            "$inc": {
                "files": files,
                **({f"features.{feature}": 1} if feature else {})
            }
        },
        upsert=True
    )
...
def inc_daily(uid, feature=None, files=0):
    day = today_key().strftime("%Y-%m-%d")

    update = {
        "$addToSet": {"active_users": uid},
        "$inc": {"files": files}
    }

    if feature:
        update["$inc"][f"features.{feature}"] = 1

    col_usage_daily.update_one(
        {"_id": day},
        update,
        upsert=True
    )
...
async def handler(event):
    ...
    if state == "WAIT_CONTACT_BASE":
        ...
        # ✅ ONLY CHANGE — Case-1
            ...
            os.remove(out)
            inc_total(feature="edit")
            inc_daily(uid, feature="edit",             files=len(user_files.get(uid, [])))
    ...
    if state == "WAIT_SPLIT_COUNT":
        ...
        for i in range(0, len(cards), count):
            ...
            os.remove(out)
            inc_total(feature="split")
            inc_daily(uid, feature="split", files=1)
...
# ---------- MAKE VCF ----------
   ...
   # ---------- TXT TO VCF ----------
    if state == "WAIT_TXT_FILE":
        for src in user_files[uid]:
            ...
            await client.send_file(chat, out)
            os.remove(out)
            inc_total(feature="txt2vcf")
            inc_daily(uid, feature="txt2vcf", files=1)
    ...
    if state == "WAIT_CSV_FILE":
        for src in user_files[uid]:
            ...
            await client.send_file(chat, out)
            os.remove(out)
            inc_total(feature="csv2vcf")
            inc_daily(uid, feature="csv2vcf", files=1)
 ...
 # ---------- SPLIT (PART) ----------
    ...
    if  n.strip()]
        ...
        await client.send_file(chat, out)
        os.remove(out)
        inc_total(feature="admin_navy")
        inc_daily(uid, feature="admin_navy")
...
async def done(event):
    ...
    if user_state.get(uid) == "WAIT_EDIT_FILES":
        ...
        out = "merged.vcf"
        inc_total(feature="merge")
        inc_daily(uid, feature="merge", files=1)
        with open(out, "w") as w:
    ...
    if state == WAIT_ANALYZE_FILE:
        ...
        for i, path in enumerate(files, start=1):
            try:
                ...
                await event.reply(f"```\n{header}{report}\n```")
                inc_total(feature="analyze")
                inc_daily(uid, feature="analyze", files=1)
...
async def health(event):
    ...
    # 1️⃣ SEND PROCESSING MESSAGE
    processing = await event.reply("⏳ **Processing health report...**")
    ...
    try:
        ...
        try:
            mem = round(psutil.virtual_memory().used / (1024 * 1024), 1)
        except:
            mem = "N/A"
        ...
        # ========= DB SAFE =========
        try:
            errors_today = (col_errors.find_one({"_id": today}) or {}).get("count", 0)

            total = col_usage_total.find_one({"_id": "total"}) or {}
            total_files = total.get("files", 0)
            total_features = total.get("features", {})

            daily = col_usage_daily.find_one({"_id": today}) or {}
            today_users = len(daily.get("active_users", []))
            today_files = daily.get("files", 0)
            today_features = daily.get("features", {})
        ...
        report = (
            ...
            f"⚠️ Errors Today   : {errors_today}\n"
            f"🕒 Server Time    : {now.strftime('%d %b %Y | %I:%M %p')}\n\n"

            "📊 **TOTAL USAGE**\n"
            "━━━━━━━━━━━━━━━━━━━━━━\n"
            f"📂 Total Files    : {total_files}\n"
            f"{fmt(total_features)}\n"

            "📅 **TODAY'S USAGE**\n"
            "━━━━━━━━━━━━━━━━━━━━━━\n"
            f"👥 Active Users   : {today_users}\n"
            f"📂 Files Today   : {today_files}\n"
            f"{fmt(today_features)}"
        )

        # 2️⃣ EDIT PROCESSING MESSAGE → FINAL REPORT
        await processing.edit(report)

    except Exception as e:
        await processing.edit("❌ **Health report generate karne me error aaya.**")
