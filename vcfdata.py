...
# ================= BUTTON BROADCAST SYSTEM =================


WAIT_BUTTON_MSG = "WAIT_BUTTON_MSG"
WAIT_BUTTON_NAME = "WAIT_BUTTON_NAME"
WAIT_BUTTON_LINK = "WAIT_BUTTON_LINK"
WAIT_BUTTON_PREVIEW = "WAIT_BUTTON_PREVIEW"
WAIT_BUTTON_BROADCAST = "WAIT_BUTTON_BROADCAST"
...
async def button_flow(event):
    ...
    # ---------- MESSAGE ----------
    if state == WAIT_BUTTON_MSG:
        user_data[uid]["btn_msg"] = text
        user_state[uid] = WAIT_BUTTON_NAME
        await event.reply("🔘 Send button name")
        return

    # ---------- BUTTON NAME ----------
    if state == WAIT_BUTTON_NAME:
        user_data[uid]["btn_name"] = text
        user_state[uid] = WAIT_BUTTON_LINK
        await event.reply("🔗 Send button link")
        return

    # ---------- BUTTON LINK ----------
    if state == WAIT_BUTTON_LINK:
        user_data[uid]["btn_link"] = text
        user_state[uid] = WAIT_BUTTON_PREVIEW

        msg = user_data[uid]["btn_msg"]
        name = user_data[uid]["btn_name"]
        link = user_data[uid]["btn_link"]

        await event.reply(
            "👀 Preview below — confirm broadcast?",
            buttons=[
                [Button.inline("✅ Yes Broadcast", b"preview_yes")],
                [Button.inline("❌ Cancel", b"preview_cancel")]
            ]
        )

        await event.reply(msg, buttons=[[Button.url(name, link)]])
        return
        
@client.on(events.CallbackQuery)
async def button_callbacks(event):
    uid = event.sender_id
    ...
    # ---------- PREVIEW YES ----------
    if data == "preview_yes":
        user_state[uid] = WAIT_BUTTON_BROADCAST

        await event.edit(
            "📢 Select broadcast option",
            buttons=[
                [Button.inline("📢 Group & Channel", b"bc_gc")],
                [Button.inline("👥 Total Members", b"bc_users")],
    ...
    name = user_data[uid]["btn_name"]
    link = user_data[uid]["btn_link"]
    btn = [[Button.url(name, link)]]

    # ---------- GROUP & CHANNEL ----------
    if data == "bc_gc":
        for chat_id in GROUP_CHANNEL_IDS:
            try:
                await client.send_message(chat_id, msg, buttons=btn)
            except:
                pass

        await event.edit("✅ Sent to group & channel")
        user_state[uid] = None
        return
    ...
    if data == "bc_users":
        ...
        for user in users:
            try:
                await client.send_message(user["user_id"], msg, buttons=btn)
            except:
                pass

        await event.edit("✅ Sent to all users")
        user_state[uid] = None
        return
...
async def button_start(event):
    if event.sender_id != ADMIN_ID:
        return

    uid = event.sender_id
    user_state[uid] = WAIT_BUTTON_MSG
    user_data[uid] = {}

    await event.reply("✏ Send message for button post")
