...
WAIT_SPLIT_BASE_NAME = "WAIT_SPLIT_BASE_NAME"
...
# safe filename (spaces remove optional)
    ...
    if state == WAIT_SPLIT_BASE:
            ...
            await event.reply("👤 Send Contact  Name (example- Spidy)")
            return
  ...
  # ---------- SPLIT ----------
    ...
    if state == WAIT_SPLIT_CONTACT_NAME:
        ...
        if total == 0:
            await event.reply("❌ No contacts found")
            return
 ...
 # ---------- SPLIT (PART) ----------
    ...
    if state == WAIT_PART_SPLIT_COUNT:
        ...
        await event.reply(
            ...
            buttons=[
                [Button.inline("✅ Yes", "split_custom_yes")],
                [Button.inline("❌ No", "split_custom_no")]
            ]
        )
    ...
    if state == WAIT_SPLIT_BASE_NAME:

        base_name = text.strip()

        if not base_name:
            await event.reply("❌ Send valid base name")
            return

        user_data[uid]["split_base"] = base_name
        ...
        for i in range(parts):
            ...
            with open(out, "w", encoding="utf-8") as w:
                ...
                for card in cards[index:index + take]:
                    ...
                    name = f"{contact_base}{counter:02}"
                    counter += 1
    ...
    # CONTACT RANGE CALCULATION
        min_contacts = chunk
        max_contacts = chunk + (1 if extra > 0 else 0)

        await event.reply(
            "╔══════════════════╗\n"
            "   ✅ CUSTOM SPLIT COMPLETED\n"
            "╚══════════════════╝\n\n"
            f"📁 Base File Name: {base_file}\n"
            f"👤 Contact Base Name: {contact_base}\n\n"
            f"📇 Total Contacts: {total}\n"
            f"📂 Total VCF Files: {parts}\n"
            f"📦 Contacts Per VCF: {min_contacts} - {max_contacts}\n\n"
            + "\n".join(summary)
        )
...
async def done(event):
    ...
    # ===== SPLIT FLOW =====
    elif user_state.get(uid) == "WAIT_SPLIT_FILE":
