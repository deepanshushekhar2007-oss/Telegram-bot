...
def get_config(key, default=None):
    data = col_config.find_one({"_id": key})
    return data["value"] if data else default

# delete old temp files if bot restarted
for f in os.listdir():
...
FORCE_SUB_CHANNEL = "SPIDY_W_S"   # username only (no @)
FORCE_SUB_FILE = "force_sub.json"
force_sub_enabled = False

# ================= INIT =================
client = TelegramClient("spidy_vcf_bot", api_id, api_hash)
...
➤ **/access USER_ID DAYS (Names)**
   ┗ 🎟️ Grant subscription access (user menu name)

➤ **/access on**
   ┗ 🆓 Enable FREE MODE (all users unlocked)

➤ **/access off**
   ┗ 🔒 Disable FREE MODE (subscription required)
...
➤ **/admin**
   ┗ 🛠️ Show admin command panel

➤ **/status**
   ┗ 🖥️ Check bot status & uptime

➤ **/info all**
   ┗ 📊 Full report (active, expired & banned users)
...
➤ **/users**
   ┗ 👥 Total users count (before broadcast)
   
➤ **/all**
   ┗ 👥 All names of total users 
 
➤ **/names**
   ┗ 👑 List of all users who have a custom names    ┗ Shows User ID + Username + Custom Name

➤ **/removename** USER_ID
   ┗ 🗑️ Remove custom name of a specific user
   ┗ User will return to default name after removal

➤ **/report**
   ┗ 📁 Export full user & subscription report
   
➤ **/button**
   ┗ 📢 Create button msg + broadcast post
...
➤ **/done**
   ┗ 📂 Finish file upload & start processing

➤ **/help**
   ┗ ℹ️ Complete user guide
...
✨ Advanced VCF Editor  
✨ Split VCF & Split VCF (In Parts)  
✨ Universal File Converter  
✨ Smart File Analyzer  
✨ Premium & VIP Tools  

➡️ **All features are accessible via inline buttons**
...
# SPLIT VCF
WAIT_SPLIT_FILE = "WAIT_SPLIT_FILE"
WAIT_SPLIT_CONTACTS = "WAIT_SPLIT_CONTACTS"
WAIT_SPLIT_BASE = "WAIT_SPLIT_BASE"
WAIT_SPLIT_CONTACT_NAME = "WAIT_SPLIT_CONTACT_NAME"

# SPLIT VCF PART
WAIT_PART_SPLIT_FILE = "WAIT_PART_SPLIT_FILE"
WAIT_PART_SPLIT_COUNT = "WAIT_PART_SPLIT_COUNT"
WAIT_SPLIT_CUSTOM_CONFIRM = "WAIT_SPLIT_CUSTOM_CONFIRM"
WAIT_PART_BASE_NAME = "WAIT_PART_BASE_NAME"
WAIT_PART_CONTACT_NAME = "WAIT_PART_CONTACT_NAME"
...
def load_users_from_mongo():
    ...
    for u in col_users.find({}, {"user_id": 1}):
        all_users.add(u["user_id"])

load_users_from_mongo()

def is_subscribed(uid):
    # 🔥 FIRST check real subscription
    data = col_subs.find_one({"user_id": uid})

    if data:
        expiry = data["expiry"]
        if datetime.now() > expiry:
            col_subs.delete_one({"user_id": uid})
            return False
        return True   # ✅ subscription hai → always subscribed

    # 🆓 Agar subscription nahi hai tab free mode apply karo
    if free_mode:
        return True
...
def is_banned(uid):
    return col_bans.find_one({"user_id": uid}) is not None
...
async def show_menu(chat):
    user_id = chat
    default_name = "SPIDY VCF EDITOR"

    user_data = col_subs.find_one({"user_id": user_id})

    bot_name = default_name  # default fallback
    ...
    if user_data:
        ...
        if expiry and expiry < datetime.now():
            col_subs.delete_one({"user_id": user_id})
        else:
            custom_name = user_data.get("name")
            if custom_name:
...
def unban_user(uid):
    col_bans.delete_one({"user_id": uid})
    
def set_subscription(uid, expiry):
    col_subs.update_one(
        {"user_id": uid},
        {"$set": {"expiry": expiry}},
        upsert=True
...
def get_subscription(uid):
    data = col_subs.find_one({"user_id": uid})
    return data["expiry"] if data else None
...
async def start(event):
    ...
    # =========================
    # 🚫 BAN CHECK (SABSE PEHLE)
    # =========================
    if uid in banned_users:

        # 🔔 Admin notify background me
        asyncio.create_task(
            notify_admin(
                "🚫 **BANNED USER TRYING /START**\n\n"
                f"🆔 User ID: `{uid}`\n"
        ...
        await event.reply(
            ...
            "   🔒 ACCOUNT RESTRICTED\n"
            "━━━━━━━━━━━━━━━━━━━━━━\n\n"
            "🚫 Access to this bot has been revoked.\n"
            "🛑 Usage is temporarily or permanently blocked.\n\n"
            "📩 Reach out to admin if needed."
        )
        return
    ...
    col_users.update_one(
        {"user_id": uid},
        {"$set": {"user_id": uid, "first_seen": datetime.now()}},
        upsert=True
    )
    ...
    # =========================
    sub_data = col_subs.find_one({"user_id": uid})
    subscribed = False
    ...
    # =========================
    # 4️⃣ SUBSCRIPTION BLOCK
    # =========================
    if not subscribed and not free_mode:
        await event.reply(
            "╭━━━ 〔💎 PREMIUM REQUIRED〕━━━╮\n\n"
            "🚫 Status  : Inactive\n"
            "🔒 Features: Locked\n\n"
            "✨ Upgrade to unlock unlimited access.\n"
            "💳 Purchase Now 👉 @SPIDYWS\n"
            "╰━━━━━━━━━━━━━━━━━━━━━━━━━╯"
        )
        return

    # =========================
    # 5️⃣ SHOW MAIN MENU (USER KO FAST RESPONSE)
    # =========================
    init_user(uid)
...
async def user_help(event):
    text = (
        ...
        "║ • Select feature from menu     \n"
        "║ • Upload file (VCF/TXT/CSV)    \n"
...
async def handler(event):
    ...
    # ---------- BUTTON FLOW ----------
    if uid in button_state:

        # TEXT stage
        if button_state[uid] == "WAIT_BUTTON_TEXT":
            button_data[uid] = {"text": event.raw_text}
            button_state[uid] = "WAIT_BUTTON_NAME"
            await event.reply("🔘 Button ka NAME bhejo")
            return

        # NAME stage
        if button_state[uid] == "WAIT_BUTTON_NAME":
            button_data[uid]["name"] = event.raw_text
            button_state[uid] = "WAIT_BUTTON_LINK"
            await event.reply("🔗 Button ka LINK bhejo")
            return

        # LINK stage
        if button_state[uid] == "WAIT_BUTTON_LINK":
            button_data[uid]["link"] = event.raw_text

            text = button_data[uid]["text"]
            name = button_data[uid]["name"]
            link = button_data[uid]["link"]

            await event.respond(
                text,
                buttons=[[Button.url(name, link)]]
            )

            del button_state[uid]
            del button_data[uid]
            return
    ...
    # 🤖 AGENT MODE FILE
        if state == "WAIT_AGENT_FILE":
            import uuid
 
            original_name = event.file.name or "file"
            user_data[uid]["original_name"] = original_name
  
            ext = os.path.splitext(original_name)[1]
            file_path = f"temp_{uuid.uuid4().hex}{ext}"
            ...
            user_data[uid]["agent_numbers"] = numbers
 
            await event.reply(
                f"📁 File `{original_name}` has been uploaded.\n"
                f"📊 Number of Contacts: {total}\n\n"
                "Reply in this format:\n"
                "<contact name>,<vcf file name>,<contact count>,<start number>\n\n"
                "Example:\n"
                "SPIDY,VIPFile,100,1"
            )

            user_state[uid] = "WAIT_AGENT_FORMAT"
            return   # ⛔ YAHAN RETURN HONA ZARURI HAI

    # 📂 NORMAL FILE FLOW (existing logic)
        import uuid

        original_name = event.file.name or "file"

# agar koi filename nahi ho to default
        if not original_name:
            original_name = "file"

# safe filename (spaces remove optional)
        original_name = os.path.    basename(original_name)
 
        file_path = original_name  # 👈 yahi main change hai
    ...
    if state == WAIT_SPLIT_CONTACTS:
        ...
        if per_file <= 0:
            await event.reply("❌ Number must be greater than 0")
            return

        user_data[uid]["per_file"] = per_file
        user_state[uid] = WAIT_SPLIT_BASE

        await event.reply("📁 Send Base File Name")
        return


# BASE FILE NAME
    if state == WAIT_SPLIT_BASE:

        user_data[uid]["base_name"] = text.strip()

        user_state[uid] = WAIT_SPLIT_CONTACT_NAME
        await event.reply("👤 Send Contact Base Name")
        return


 # CONTACT NAME + SPLIT
    if state == WAIT_SPLIT_CONTACT_NAME:

        try:

            contact_base = text.strip()

            src = user_data[uid]["split_file"]
            per_file = user_data[uid]["per_file"]
            base = user_data[uid]["base_name"]

            if not os.path.exists(src):
                await event.reply("❌ VCF file not found")
                return

            if per_file <= 0:
                await event.reply("❌ Contacts per file must be greater than 0")
                return

            with open(src, "r", errors="ignore") as f:
                cards = [c for c in f.read().split("END:VCARD") if "BEGIN:VCARD" in c]

            total = len(cards)

            if total == 0:
                await event.reply("❌ No contacts found in VCF")
                return
        
            files = math.ceil(total / per_file)
            ...
            for i in range(files):
                ...
                chunk = cards[index:index+per_file]
                out = f"{base}{i+1:02}.vcf"

                with open(out, "w", encoding="utf-8") as w:

                    for card in chunk:

                        name = f"{contact_base}{counter:03}"
                        counter += 1
                ...
                summary.append(f"📂 {out} → {len(chunk)} contacts")
...
📁 Base File Name: {base}
👤 Contact Base Name: {contact_base}

📇 Total Contacts: {total}
📂 Total Files: {files}
📦 Contacts Per File: {per_file}
...
{msg}
    ...
    # BASE NAME
    if state == WAIT_PART_BASE_NAME:

        user_data[uid]["split_base"] = text.strip()

        user_state[uid] = WAIT_PART_CONTACT_NAME
...
# CONTACT NAME
    if state == WAIT_PART_CONTACT_NAME:

        contact_base = text.strip()

        src = user_data[uid]["split_file"]
        parts = user_data[uid]["split_parts"]
        base_file = user_data[uid]["split_base"]
        ...
        for i in range(parts):
            ...
            out = f"{base_file}{i+1:02}.vcf"

            with open(out, "w", encoding="utf-8") as w:

                for card in cards[index:index+take]:
            ...
            summary.append(f"📂 {out} → {take} contacts")
        ...
        await event.reply(
    f"""╔══════════════════╗
       ✅ CUSTOM SPLIT COMPLETED
        ╚══════════════════╝

    📁 Base File Name: {base_file}
    👤 Contact Base Name: {contact_base}

    📇 Total Contacts: {total}
    📂 Total VCF Files: {parts}
    ...
    if state == "WAIT_AGENT_FORMAT":
        try:
            contact_name, vcf_name, count, start = [x.strip() for x in text.split(",")]
            count = int(count)
            start = int(start)
        except:
            await event.reply("❌ Invalid format.\nUse:\nName,FileName,Count,StartNumber")
            return

        numbers = user_data[uid].get("agent_numbers", [])

        if not numbers:
            await event.reply("❌ No numbers found.")
            cleanup(uid)
            return

        if count <= 0:
            await event.reply("❌ Contact count must be greater than 0.")
            return

        total_numbers = len(numbers)
        total_files = (total_numbers + count - 1) // count

    # 🔥 SAVE DATA FOR CONFIRM STEP
        user_data[uid]["agent_config"] = {
    ...
    # 📊 REPORT + BUTTONS
        await event.reply(
            ...
            f"📦 Contacts Per VCF: {count}\n"
            f"📊 Total Numbers Found: {total_numbers}\n"
            f"📁 Total VCF Files Will Be Created: {total_files}\n"
        ...
        user_state[uid] = "WAIT_AGENT_CONFIRM"
        return
...
async def buttons(event):
    ...
    if data == "edit":
        ...
        await event.edit(
            "📝 Send your VCF file to start editing.\n\n"
            "📌 After uploading, type /done to continue.",
            buttons=None
        )
        return
    ...
    if data == "split":
        user_state[uid] = "WAIT_SPLIT_FILE"
        user_files[uid] = []

        await event.edit(
            "✂️ Send your VCF file to split into multiple files.\n\n"
            "📌 After uploading, type /done to continue.",
            buttons=None
        )
        return
    ...
    if data == "merge":
        ...
        await event.edit(
            "🔗 Upload the VCF files you want to merge.\n\n"
            "📌 After uploading all files, type /done to continue.",
            buttons=None
        )
        return
    ...
    if data == "addnum":
        ...
        await event.edit(
            "➕ Upload one VCF file to add numbers.\n\n"
            "📌 After uploading, type /done to continue.",
            buttons=None
        )
        return
    ...
    if data == "makevcf":
        ...
        await event.edit(
            "📱 Send phone numbers (one per line) to create a VCF file.",
            buttons=None
        )
        return
    ...
    if data == "splitpart":
        ...
        await event.edit(
            "🪓 Upload one VCF file to split by parts.\n\n"
            "📌 After uploading, type /done to continue.",
            buttons=None
        )
        return
    ...
    if data == "admin_navy":
        ...
        await event.edit(
            "👮‍♂️ Send ADMIN phone numbers (one per line).\n\n"
            "📌 Type 'skip' if you don’t have ADMIN numbers.",
            buttons=None
        )
        return
    ...
    if data == "converter":
        ...
        await event.edit(
            "🔄 Upload files (TXT / VCF / CSV / XLSX) to convert.\n\n"
            "📌 After uploading all files, type /done to continue.",
            buttons=None
        )
        return
    ...
    if data == "agent_mode":
        user_state[uid] = "WAIT_AGENT_FILE"
        user_files[uid] = []

        await event.edit(
            "🤖 Agent Mode activated.\n\n"
            "📤 Upload a file (VCF / TXT / CSV / XLSX) to process.",
            buttons=None
        )
        return
...
# ---------- CONVERT (ALL TYPES / ALL FILES) ----------
    ...
    # 2️⃣ CONVERT ALL FILES
        ...
        # VCF (even vcf → vcf allowed)
            ...
            os.remove(path)   # temp input file delete
            os.remove(out)    # converted file bhi delete after sending
...
# YES
    if data == "split_custom_yes":
        ...
        user_state[uid] = WAIT_PART_BASE_NAME
...
# NO
    if data == "split_custom_no":
        ...
        for i in range(parts):
            ...
            with open(out, "w") as w:
                ...
                for c in cards[index:index+take]:
                    w.write(c.strip()+"\nEND:VCARD\n")
        ...
        await event.reply("✅ Split completed")
    ...
    if data == "agent_confirm":
        if user_state.get(uid) !=     "WAIT_AGENT_CONFIRM":
            return

        config = user_data[uid].get("agent_config")
        numbers = user_data[uid].get("agent_numbers")

        if not config or not numbers:
        ...
        progress_msg = await event.edit("⏳ Generating VCF files...\n\n0%")
        ...
        while current_index < total_numbers:
            ...
            chunk = numbers[current_index: current_index + count]
            out = f"{vcf_name}{file_index}.vcf"
            ...
            with open(out, "w") as f:
                for num in chunk:
                    ...
                    f.write(
                        "BEGIN:VCARD\n"
                        "VERSION:3.0\n"
                        f"N:{contact_name}{counter:03};;;;\n"
                        f"FN:{contact_name}{counter:03}\n"
                        f"TEL:{formatted}\n"
                        "END:VCARD\n"
                    )
        ...
        # 🔥 PROGRESS CALCULATION
            percent = int((file_index - 1) / total_files * 100)
            bar_length = 20
            ...
            await progress_msg.edit(
                ...
                f"[{bar}] {percent}%\n\n"
                f"📁 {file_index - 1}/{total_files} files completed"
            )

        await progress_msg.edit("📤 Uploading files...")
...
async def done_handler(event):
    ...
    # ===== EDIT FLOW =====
    if state == "WAIT_EDIT_FILES":

        if not user_files.get(uid):
            await event.reply("❌ No VCF uploaded.")
            return
    ...
    # ===== SPLIT VCF =====
    elif state == WAIT_SPLIT_FILE:

        if not user_files.get(uid):
            await event.reply("❌ No VCF uploaded")
            return

        user_data[uid]["split_file"] = user_files[uid][0]

        user_state[uid] = WAIT_SPLIT_CONTACTS
        await event.reply("🔢 Send contacts per file")
        return


    # ===== SPLIT VCF PARTS =====
    elif state == WAIT_PART_SPLIT_FILE:
        ...
        await event.reply("🔢 Into how many parts do you want to split?")
        return


    # ===== ANALYZE FILE =====
    elif state == WAIT_ANALYZE_FILE:
...
async def info_all(event):
    ...
    msg = "📋 **FULL BOT USER REPORT**\n\n"

    # =========================
    # 🟢 ACTIVE SUBSCRIPTIONS
    # =========================
    active_cursor = col_subs.find({"expiry": {"$gt": now}})
    active_list = list(active_cursor)
    ...
    # 🧹 EXPIRED USERS
    # =========================
    expired_cursor = col_subs.find({"expiry": {"$lte": now}})
    expired_list = list(expired_cursor)
    ...
    if expired_list:
        for sub in expired_list:
            ...
            msg += (
                f"👤 {uname}\n"
                f"🆔 {uid}\n"
                f"⌛ Expired on: {expiry.strftime('%d %b %Y | %I:%M %p')}\n"
                f"──────────────\n"
            )
    else:
    ...
    # 🚫 BANNED USERS
    # =========================
    banned_cursor = col_bans.find({})
    banned_list = list(banned_cursor)
...
async def access(event):
    ...
    try:
        ...
        if len(parts) == 2 and parts[1].lower() in ["on", "off"]:
            ...
            free_mode = True if parts[1].lower() == "on" else False

            # ✅ Save in MongoDB
            set_config("free_mode", free_mode)

            await event.reply(
                f"✅ FREE MODE {'ENABLED' if free_mode else 'DISABLED'}"
            )
            return
        ...
        await client.send_message(
            ...
            "📂 Send /start to access your dashboard."
        )
        ...
        await event.reply(
            ...
            "/access on\n"
            "/access off\n"
            "/access USER_ID DAYS (NAME optional)"
        )
...
async def force_sub_control(event):
    ...
    if parts[1].lower() == "on":
        ...
        # ✅ Save in MongoDB
        set_config("force_sub", force_sub_enabled)
...
async def broadcast(event):
    ...
    # ✅ MongoDB se users load
    users_cursor = col_users.find({})
...
async def ban_user(event):
    ...
    # ✅ Admin confirmation
    await event.reply(f"🚫 USER BANNED\n\n🆔 User ID: `{uid}`")
...
async def unban_user(event):
    ...
    if uid not in banned_users:
        await event.reply("⚠️ User already unbanned.")
        return
...
async def show_all_users(event):
    ...
    if uid != ADMIN_ID:
        return await event.reply("❌ Only admin allowed")

    # ===== TOTAL USERS COUNT =====
    total = col_users.count_documents({})

    if total == 0:
        return await event.reply("⚠️ No users found")

    # ===== START MESSAGE =====
    msg = await event.reply("⏳ Fetching users...\n\n[░░░░░░░░░░] 0%")

    users_cursor = col_users.find({}, {"_id": 0, "user_id": 1})
    ...
    # ===== FETCH LOOP =====
    async for u in async_cursor(users_cursor):
    ...
    # ==============================
    # IF USERS > 100 → TXT EXPORT
    # ==============================
    if total > 100:
        file_name = "all_users.txt"

        with open(file_name, "w", encoding="utf-8") as f:
            f.write("ALL REGISTERED USERS\n")
            f.write("=====================\n\n")
            for line in lines:
                f.write(line)
            f.write(f"\nTotal Users : {total}")
        ...
        await client.send_file(
            event.chat_id,
            file_name,
            caption=f"👥 Total Users : {total}\n📄 User list exported"
        )
        return

    # ==============================
    # SHOW IN CHAT (SAFE SPLIT)
    # ==============================
    header = (
        "╔════════════════════╗\n"
    ...
    full_text = header + "".join(lines) + f"\n📊 Total Users : {total}"

    MAX = 4000  # telegram safe limit
    ...
    if len(full_text) <= MAX:
        ...
        for i in range(0, len(full_text), MAX):
            await event.reply(full_text[i:i+MAX])


# ==============================
# HELPER ASYNC CURSOR
# ==============================
async def async_cursor(cursor):
    for item in cursor:
        yield item
...
async def total_users(event):
    ...
    # ✅ MongoDB se total users
    total_users = col_users.count_documents({})

    # optional — agar subscription Mongo me shift karna ho
    active_subs = col_subs.count_documents({
        "expiry": {"$gt": datetime.now()}
    })

    expired_subs = col_subs.count_documents({
    ...
    await event.reply(
        ...
        "💳  𝗦𝗨𝗕𝗦𝗖𝗥𝗜𝗣𝗧𝗜𝗢 𝗡 𝗦𝗧𝗔𝗧𝗨𝗦\n"
        "━━━━━━━━━━━━━━━━━━━━━━\n"
        f"🟢 Active Users   : {active_subs}\n"
        f"🔴 Expired Users  : {expired_subs}\n\n"
...
async def system_status(event):
    ...
    # 🔹 Load Config
    free_mode_status = get_config("free_mode", False)
    force_sub_status = get_config("force_sub", False)

    # 🔹 Premium Count
    now = datetime.now()
    premium_count = col_subs.count_documents({
        "expiry": {"$gt": now}
    })
    ...
    await event.reply(
        ...
        "⚙ SYSTEM SETTINGS\n"
        f"🔓 FREE MODE      : {'ON ✅' if free_mode_status else 'OFF ❌'}\n"
        f"📢 FORCE SUB      : {'ON ✅' if force_sub_status else 'OFF ❌'}\n\n"
...
async def create_button(event):
    if event.sender_id != ADMIN_ID:
        return

    uid = event.sender_id
    button_state[uid] = "WAIT_BUTTON_TEXT"

    await event.reply(
        "📝 Button message ka TEXT bhejo\n"
        
    )
    
    
@client.on(events.NewMessage(pattern="/names"))
async def list_custom_names(event):
    if event.sender_id != ADMIN_ID:
        return

    users = col_subs.find({"name": {"$exists": True, "$ne": None}})

    text = "👑 CUSTOM NAME USERS\n\n"
    found = False
    ...
    for user_data in users:
        ...
        # get telegram username safely
        try:
            tg_user = await client.get_entity(uid)
        ...
        text += (
            f"🆔 ID      : {uid}\n"
            f"👤 Username: {username}\n"
            f"🏷️ Name    : {custom_name}\n"
            f"━━━━━━━━━━━━━━━━\n"
        )
        found = True

    if not found:
        await event.reply("❌ No custom names found")
        return

    text += "\n🗑️ Name remove karne ke liye use kare:\n/removename USER_ID"

    await event.reply(text)
    
    
@client.on(events.NewMessage(pattern="/removename"))
async def remove_name(event):
    if event.sender_id != ADMIN_ID:
        return
    ...
    try:
        ...
        result = col_subs.update_one(
            {"user_id": uid},
            {"$unset": {"name": ""}}
        )

        if result.modified_count:
            await event.reply("✅ Custom name removed successfully")
        else:
            await event.reply("❌ No custom name found for this user")

    except:
        await event.reply("❌ Usage: /removename USER_ID")
