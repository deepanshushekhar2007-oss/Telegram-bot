...
async def show_menu(chat):
    ...
    # 🗑️ DELETE OLD MENU IF EXISTS
    try:
        if user_id in last_menu_message:
            await client.delete_messages(chat, last_menu_message[user_id])
    except:
        pass
    ...
    # 💾 SAVE NEW MESSAGE ID
    last_menu_message[user_id] = msg.id
...
async def done(event):
    ...
    if user_state.get(uid) == "WAIT_EDIT_FILES":
        ...
        await event.reply("📊 How many contacts per file? (example: 50)")
        return


    elif user_state.get(uid) == "WAIT_SPLIT_COUNT":
    ...
    elif user_state.get(uid) ==     "WAIT_SPLIT_BASENAME":
