...
# safe filename (spaces remove optional)
  ...
  # ---------- SPLIT ----------
    ...
    if user_state.get(uid) == "WAIT_SPLIT_COUNT":
        ...
        try:
            ...
            user_state[uid] = "WAIT_SPLIT_BASENAME"

            await event.reply("📝 Send base file name (example: VIP)")
            return
    ...
    if user_state.get(uid) == "WAIT_SPLIT_BASENAME":

        user_data[uid]["base_name"] = event.raw_text.strip()
        user_state[uid] = "WAIT_SPLIT_CONTACT_NAME"

        await event.reply("👤 Send contact base name (example: spidy)")
        return
    
    
    if state == "WAIT_SPLIT_CONTACT_NAME":

        base_contact_name = event.raw_text.strip()
        data = user_data[uid]

        file_path = data["split_file"]
        split_count = data["split_count"]
        base_name = data["split_base"]
    ...
    # 🔥 AUTO DIGIT DETECT
        ...
        for i in range(0, total, split_count):
            chunk = contacts[i:i+split_count]
            out_file = f"{base_name}_{file_number}.vcf"

            with open(out_file, "w", encoding="utf-8") as w:
                for contact in chunk:

                    number = str(global_counter).zfill(digit_length)
                    new_name = f"{base_contact_name}{number}"
                    ...
                    for line in lines:
                        if line.startswith("FN:"):
                            new_contact.append(f"FN:{new_name}")
                        elif line.startswith("N:"):
                            new_contact.append(f"N:{new_name};;;;")
                        else:
