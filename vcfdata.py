...
async def show_all_users(event):
    ...
    msg = await event.reply("⏳ Processing users...\n\n[░░░░░░░░░░] 0%")
    ...
    for bar, delay in steps:
        await asyncio.sleep(delay)
        await msg.edit(f"⏳ Processing users...\n\n{bar}")

    # ===== GET USERS =====
    users = get_all_users()   # <-- apna database function

    if not users:
        return await msg.edit("⚠️ No users found")

    # ===== BUILD RESULT TEXT =====
    text = (
        "╔════════════════════╗\n"
        "   👥 𝗔𝗟𝗟 𝗥𝗘𝗚𝗜𝗦𝗧𝗘𝗥𝗘𝗗 𝗨𝗦𝗘𝗥𝗦\n"
        "╚════════════════════╝\n\n"
    )
