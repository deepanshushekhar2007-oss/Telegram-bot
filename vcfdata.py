...
async def user_help(event):
    text = (
        ...
        "➤ **Choose option from buttons**\n"
        "┗ 👆 EDIT / SPLIT / SPLIT PART / MERGE / CONVERTER / ANALYZE\n\n"
        ...
        "✏️ **EDIT VCF**\n"
        "┗ Rename contacts & files automatically\n\n"

        "🔪 **SPLIT VCF**\n"
        "┗ Split VCF by fixed number of contacts\n"
        "┗ Example: 1000 contacts per file\n"
        "┗ Multiple output VCF files\n\n"

        "📌 **SPLIT VCF (IN PARTS)**\n"
        "┗ Split one VCF into custom number of parts\n"
        "┗ Example: 5 equal parts\n"
        "┗ Contacts auto-balanced\n"
        "┗ Output provided as ZIP file\n\n"
        ...
        "┗ CSV ⇄ VCF\n"
        "┗ XLSX ⇄ TXT\n"
        "┗ ZIP → Auto extract & convert\n\n"

        "━━━━━━━━━━━━━━━━━━━━━━\n"
        "🔍 **ANALYZE FILE**\n\n"

        "┗ File name & size\n"
        "┗ Total contacts & numbers\n"
        ...
        "👮‍♂️⚓ **ADMIN + NAVY VCF**\n"
        "┗ Combined ADMIN & NAVY contacts\n"
        "┗ Fixed naming (ADMIN01…, NAVY01…)\n"
        ...
        "➤ **/subscription**\n"
        "┗ Check active plan & remaining time\n\n"
...
async def handler(event):
    ...
    # link validation (VERY IMPORTANT)
        if not link.startswith("http"):
            await event.reply("❌ Link https:// se start hona chahiye")
            return

        button_data[uid]["link"] = link
        button_state[uid] = "WAIT_BUTTON_TYPE"

        await event.reply(
            "⚙️ Button type choose karo",
            buttons=[
                [Button.inline("🌐 Web App", b"type_webapp")],
                [Button.inline("🔗 Default", b"type_default")]
            ]
        )
        return
...
async def buttons(event):
    ...
    data_bytes = event.data        # bytes format
    data_text = event.data.decode()   # string format

    data = data_bytes   # ⭐ IMPORTANT FIX


    # ---------------- TYPE SELECT ----------------
    if button_state.get(uid) == "WAIT_BUTTON_TYPE":

        text = button_data[uid]["text"]
        name = button_data[uid]["name"]
        link = button_data[uid]["link"]

        if data == b"type_default":
            await event.respond(
                text,
        ...
        if data == b"type_webapp":
            button_state[uid] = "WAIT_COLOR"

            await event.edit(
                "🎨 Colour choose karo",
                buttons=[
                    [Button.inline("🔴 Red", b"color_red")],
                    [Button.inline("🔵 Blue", b"color_blue")],
                    [Button.inline("🟢 Green", b"color_green")]
                ]
            )
            return
    ...
    # ---------------- COLOR SELECT ----------------
    if button_state.get(uid) == "WAIT_COLOR":
        ...
        if data == b"color_red":
            name = "🔴 " + name
        elif data == b"color_blue":
            name = "🔵 " + name
        elif data == b"color_green":
            name = "🟢 " + name
    ...
    if data == b"split_custom_no":
        ...
        cards = [c for c in data_read.split("END:VCARD") if "BEGIN:VCARD" in c]
